...
if 'authentication_status' in st.session_state and st.session_state['authentication_status']:
    ...
    def load_gradient_boosting_pipeline():
        pipeline = joblib.load('./Models/best_gbc_tuned.joblib')
        return pipeline
    
    
    @st.cache_resource(show_spinner='Models Loading')
    def load_random_forest_pipeline():
        pipeline = joblib.load('./Models/best_rf_model_and_threshold.pkl')
        return pipeline
    ...
    def select_model(key):
        ...
        with col1:
            st.selectbox('Select a model', options=['Gradient Boosting', 'Random Forest'], key=key)
        with col2:
            pass
